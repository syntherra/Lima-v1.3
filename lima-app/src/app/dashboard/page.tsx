              ...
              {metrics.insights.map((insight) => (
                <div key={insight.id} className="flex items-start space-x-3 p-4 rounded-lg border">
                  <div className="flex-shrink-0 mt-1">
                    ...
                    {insight.type === 'engagement' && <Users className="h-4 w-4 text-blue-500" />}
                    {insight.type === 'timing' && <Clock className="h-4 w-4 text-purple-500" />}
                  </div>
...
function MetricCard({ title, value, icon: Icon, trend, color, delay }: MetricCardProps) {
  const isPositive = trend > 0;
  
  const colorClasses = {
    primary: 'lima-text-blue',
  ...
  return (
    <div className="lima-metric-card lima-animate-scale-in lima-hover-lift" style={{animationDelay: `${delay}ms`}}>
      <div className="lima-card-content">
        <div className="lima-flex" style={{alignItems: 'center'}}>
          <div className="flex-shrink-0">
            <div className="p-4 rounded-xl" style={{background: 'var(--lima-gray-100)'}}>
              <Icon className={`h-8 w-8 ${colorClasses[color]}`} />
            </div>
          </div>
          <div className="ml-6 w-0 flex-1">
            <div className="lima-text-caption lima-text-secondary" style={{marginBottom: 'var(--lima-space-1)'}}>
              {title}
            </div>
            <div className="lima-flex" style={{alignItems: 'baseline'}}>
              <div className="lima-text-metric lima-text-primary">
                {value}
              </div>
              <div className={`ml-3 lima-flex lima-text-caption lima-animate-slide-right` + (isPositive ? ' text-green-600' : ' text-red-600')} style={{alignItems: 'center', fontWeight: 'var(--lima-weight-semibold)', animationDelay: `${delay + 300}ms`}}>
                {isPositive ? (
...
function formatTimeAgo(timestamp: string): string {
  ...
  if (diffInMinutes < 0) {
    ...
    if (futureMinutes < 1440) return `in ${Math.floor(futureMinutes / 60)}h`;
    return `in ${Math.floor(futureMinutes / 1440)}d`;
  }
